...
const CheckoutForm = ({serviceData}) => {
  
  const {data : userData} = useSession();
//   console.log("user :::", userData);

  const {service_id, price, title, img} = serviceData;
//   console.log("Checkout :::", serviceData);
  ...
  const onSubmit = async (formData) => {
    ...
    const payload = {...formData, service_id, service_name : title, service_image : img };
    ...
    // console.log("POSTED RES :::", postedRes);
  ...
  return (
    <div className='flex flex-col justify-center items-center m-8'>
        <Toaster />
        <h1 className='font-semibold my-4 text-xl'> Book Service - <span className='text-amber-600'> {title} </span>  </h1>
